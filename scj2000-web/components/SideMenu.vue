...
<script setup lang="ts">
    ...
    import type { GetSideMenuQuery } from '~/api/apollo'

    const { data: sideMenuData } = await useAsyncQuery<GetSideMenuQuery>(GET_SIDE_MENU)
    const sideMenuItems = computed(() => sideMenuData?.value?.sideMenu?.data?.attributes?.menuLink?.map((item: any) => item?.menuItem?.data?.attributes))
</script>
